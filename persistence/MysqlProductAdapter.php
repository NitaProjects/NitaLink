...
class MysqlProductAdapter extends MysqlAdapter {
    ...
    public function addPhysicalBook(BookPhysical $book): bool {
        ...
        try {
            ...
            // Pasar variables a bind_param
            $stmt->bind_param("isisssddddis", $productId, $author, $pages, $publisher, $publishDate, $availabilityDate, $height, $width, $length, $weight, $fragile, $isbn);
            $stmt->execute();
            $stmt->close();
    ...
    public function addDigitalBook(BookDigital $book): bool {
        ...
        try {
            ...
            $query = "INSERT INTO books_digital (product_id, author, pages, publisher, publish_date, availability_date, isbn) 
                  VALUES (?, ?, ?, ?, STR_TO_DATE(?, '%d/%m/%Y'), STR_TO_DATE(?, '%d/%m/%Y h%:m%:s'), ?)";
            $stmt = $this->connection->prepare($query);
            ...
            // Pasar variables a bind_param
            $stmt->bind_param("isisssi", $productId, $author, $pages, $publisher, $publishDate, $availabilityDate, $isbn);
            $stmt->execute();
            $stmt->close();
